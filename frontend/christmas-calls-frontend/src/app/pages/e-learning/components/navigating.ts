      ...
      <div class="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <animated-container
          *ngFor="let item of navigatingData; let index = index"
          [delay]="index * 300"
        >
          <div class="w-full h-62 rounded-2xl relative overflow-hidden">
            <img
              class="object-cover"
              [src]="item.image"
          ...
          <div class="p-3 mt-4">
            ...
            ></div>
            <button
              class="flex hover:cursor-pointer items-center gap-2 justify-between px-4 py-2 w-full shadow-stroke dark:shadow-none border-0 dark:border border-white/12 rounded-full hover:bg-surface-100 dark:hover:bg-surface-800 transition-all"
            >
