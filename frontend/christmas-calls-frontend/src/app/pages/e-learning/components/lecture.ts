...
@Component({
  ...
  template: `
    <animated-container className="mt-24 lg:mt-64 container">
      ...
      <div
        class="mt-16 relative h-128 lg:h-174 w-full rounded-2xl lg:rounded-4xl overflow-hidden shadow-black-card group"
      >
        <div
          class="absolute flex items-center justify-center inset-0 bg-[linear-gradient(0deg,rgba(0,0,0,0.24)_0%,rgba(0,0,0,0.24)_100%)]"
        >
          <button
            class="w-24 h-24 hover:cursor-pointer rounded-full flex items-center justify-center bg-white/12 group-hover:bg-white/24 transition-all backdrop-blur-[5px] border border-white/32"
            (click)="openDialog()"
          >
            <IconPlay2 />
          </button>
