...
@Component({
  ...
  template: `
    ...
    >
      <div
        *ngIf="isWide()"
        class="absolute top-0 inset-x-0 h-180 lg:h-168 shadow-black-card bg-main-gradient overflow-hidden"
      >
        <circle-pattern
          className="absolute -bottom-[115%] -right-160 w-328 lg:block hidden"
        />
      </div>
      <div class="container relative">
        <div
          *ngIf="!isWide()"
          class="absolute top-0 left-4 right-4 h-180 lg:h-168 rounded-3xl lg:rounded-4xl shadow-black-card bg-main-gradient overflow-hidden"
        >
          <circle-pattern
            class="absolute -bottom-[115%] -right-160 w-328 lg:block hidden"
          />
        </div>
        <div class="h-full relative">
          ...
          <div class="px-6 lg:px-12 mt-10 lg:mt-20 relative z-4">
            ...
            >
              Let&apos;s Shape
              <span
                class="hidden lg:inline-flex w-100 justify-between p-2 border border-white/12 rounded-full"
              >
                <IconShapesShape2 />
                <IconShapesShape3 />
                <IconShapesShape4 />
            ...
            <div
              class="group flex items-center justify-center h-128 w-full mt-12 lg:mt-10 rounded-4xl relative overflow-hidden shadow-blue-card"
            >
              <div
              ...
              />
              <button
                class="relative z-3 w-24 h-24 hover:cursor-pointer rounded-full bg-white/12 group-hover:bg-white/24 transition-all backdrop-blur-sm border border-white/32 flex items-center justify-center"
                (click)="openDialog()"
              >
                <IconPlay2 />
              </button>
