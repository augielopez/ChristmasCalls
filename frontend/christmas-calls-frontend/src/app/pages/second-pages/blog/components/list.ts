  ...
  >
    <animated-container
      *ngFor="let item of blogListData; let index = index"
      [delay]="index * 100"
      className="group relative h-136 rounded-3xl overflow-hidden shadow-blue-card dark:shadow-none border-0 dark:border border-white/12"
    >
      <img
        class="object-cover -z-2 absolute w-full h-full rounded-b-4xl"
        [src]="item.image"
        alt="Blog List Image"
      />
      <div
        class="-z-1 inset-0 absolute bg-[linear-gradient(180deg,rgba(0,0,0,0.00)_0%,rgba(0,0,0,0.48)_64.82%)] rounded-b-4xl"
      ></div>
      <div
