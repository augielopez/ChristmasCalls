...
@Component({
  ...
  template: ` <animated-container [className]="isWide() ? 'relative' : 'pt-6'">
    <div
      *ngIf="isWide()"
      class="bg-main-gradient h-206 absolute top-0 inset-x-0"
    ></div>
    <div class="container relative">
      <div
        [class]="
          'h-206 absolute top-0 left-4 right-4 ' +
          (isWide() ? '' : 'bg-main-gradient rounded-3xl lg:rounded-4xl')
        "
      >
        <div class="absolute inset-0 overflow-hidden lg:block hidden">
          <circle-pattern
            className="absolute w-328 -bottom-full translate-y-24 left-1/2 -translate-x-1/2"
          />
        </div>
      </div>
      ...
      <div
        class="relative z-10 h-140 max-w-[calc(100%-3rem)] lg:max-w-[calc(100%-12rem)] mt-16 lg:mt-40 mx-auto rounded-4xl shadow-black-card overflow-hidden"
      >
        <img
