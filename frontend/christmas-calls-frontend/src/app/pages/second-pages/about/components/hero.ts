...
@Component({
  ...
  template: `
    <animated-container [className]="isWide() ? 'relative' : 'pt-6'">
      <div
        *ngIf="isWide()"
        class="bg-main-gradient absolute top-0 inset-x-0 h-180 lg:h-206"
      ></div>
      <div class="container relative">
        <div
          [ngClass]="{
            'absolute top-0 left-4 right-4 h-180 lg:h-206': true,
            'bg-main-gradient rounded-3xl lg:rounded-4xl': !isWide(),
          }"
        >
          <div class="absolute inset-0 overflow-hidden lg:block hidden">
            <circle-pattern
              class="absolute w-328 -bottom-full translate-y-24 left-1/2 -translate-x-1/2"
            />
          </div>
        </div>
        ...
        <div
          class="relative z-10 h-140 max-w-[calc(100%-3rem)] lg:max-w-[calc(100%-12rem)] mt-12 lg:mt-48 mx-auto rounded-3xl lg:rounded-4xl shadow-black-card overflow-hidden"
        >
          <img
