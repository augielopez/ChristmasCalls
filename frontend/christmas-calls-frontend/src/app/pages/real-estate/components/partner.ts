  ...
  template: `
    <div class="container mt-24 lg:mt-32">
      <div class="w-full flex flex-wrap items-start justify-between gap-6">
        <h1
          class="max-w-136 text-3xl lg:text-6xl font-semibold text-surface-950 dark:text-surface-0 leading-tight"
        >
      ...
      <div class="mt-14 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <animated-container
          *ngFor="let item of realEstatePartnerData; let index = index"
          [delay]="index * 400"
          className="relative h-136 rounded-3xl overflow-hidden shadow-blue-card"
        >
          <img
