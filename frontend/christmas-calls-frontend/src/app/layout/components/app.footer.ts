    ...
    <animated-container>
      <footer [ngClass]="noContainer ? '' : 'container'">
        <div
          [ngClass]="
            twMerge(
              'w-full px-5 pt-5 lg:pt-22 pb-10 rounded-3xl lg:rounded-4xl overflow-hidden relative',
              image || transparent
                ? 'shadow-none'
        ...
        >
          ...
          <ng-container *ngIf="image && !transparent">
            ...
            <circle-pattern
              *ngIf="!transparent"
              className="absolute -bottom-12 opacity-50 translate-y-1/2 w-200 lg:w-7xl -translate-x-1/2 left-1/2"
            />
          </ng-container>

          <div class="max-w-5xl mx-auto flex lg:flex-row flex-col">
            <div
